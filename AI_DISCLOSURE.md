# Transparência sobre o uso de IA

Este projeto foi idealizado e dirigido por **Gustavo Martins** como uma agenda
pessoal para a Nicolly. O objetivo, a identidade afetiva, as escolhas de produto
e a autorização das mudanças são decisões humanas.

A construção do repositório contou com assistência substancial de IA generativa,
principalmente do **OpenAI Codex**, nas seguintes áreas:

- planejamento da arquitetura e dos fluxos da agenda;
- implementação e refatoração do código;
- criação e aprimoramento da interface temática;
- elaboração de testes e verificações técnicas;
- documentação e configuração do ambiente Android;
- apoio na criação e seleção de elementos visuais personalizados.

O uso de IA não elimina a necessidade de revisão humana. Resultados gerados por
IA podem conter erros, e os testes automatizados não substituem a validação final
em um aparelho Xiaomi real.

Este é um projeto pessoal, com código original publicado como open source sob a
licença MIT e finalidade não comercial. A licença do código não se estende a
marcas, personagens ou artes de terceiros. A inspiração visual em Kuromi não
representa afiliação, licença, patrocínio ou aprovação da Sanrio. Kuromi e Sanrio
pertencem aos seus respectivos titulares. Esta declaração de transparência não
concede direitos sobre marcas, personagens, imagens ou dependências de terceiros.
