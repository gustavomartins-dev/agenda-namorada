...
import type { Recommendation, RecommendationTarget } from '@/domain/recommendation';
import { colors, fonts, gradients, MIN_TOUCH_SIZE, radii, shadows, spacing } from '@/theme/tokens';

const TRACKING = {
  'agenda-namorada': 'https://github.com/gustavomartins-dev/agenda-namorada/issues/1',
  Agenda: 'https://github.com/gustavomartins-dev/Agenda/issues/7',
} as const;

export default function RecommendationsScreen() {
  const [description, setDescription] = useState('');
  const [target, setTarget] = useState<RecommendationTarget>('agenda-namorada');
  ...
  return (
    <AppScreen contentContainerStyle={styles.content} scrollProps={{ keyboardShouldPersistTaps: 'handled' }}>
      ...
      <View style={styles.card}>
        <Text style={styles.label}>Em qual agenda?</Text>
        <View style={styles.targets}>
          {(['agenda-namorada', 'Agenda'] as const).map((value) => <Pressable key={value} onPress={() => setTarget(value)} style={[styles.target, target === value && styles.targetSelected]} accessibilityRole="radio" accessibilityState={{ checked: target === value }}><Text style={[styles.targetText, target === value && styles.targetTextSelected]}>{value === 'agenda-namorada' ? 'Agenda da Nicolly' : 'Agenda do Cavaleiro'}</Text></Pressable>)}
        </View>
